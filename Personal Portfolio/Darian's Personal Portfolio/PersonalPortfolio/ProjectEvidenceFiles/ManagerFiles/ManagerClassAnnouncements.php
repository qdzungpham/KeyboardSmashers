...
// This code has the add, edit and delete functions for the student table
	...
	if (isset($_POST['addRecord'])){
		// Create the web form
		echo '
			<form method="post" action="SQLClassAnnouncements.php">
	...
	if (isset($_POST['editRecord'])){
		...
		if (empty($_POST['selectBox'])) {
			...
			$query="SELECT * FROM `classannouncements` WHERE `announcementID` = ".$announcementID.";";
	...
	if (isset($_POST['deleteRecord'])){
		...
		if (empty($_POST['selectBox'])) {
			...
			$announcementID=test_input($_POST['selectBox']);
			$query="DELETE FROM `classannouncements` WHERE `announcementID`='$announcementID'";
			$rs=$conn->prepare($query);
			$rs->execute();
